...
<?php
	$sql ='SELECT * FROM `Wine_section` ORDER BY timestamp DESC limit 9';//搜尋語句
...
<html lang="zh_tw">
  ...
  <head>
    <meta charset="utf-8">
    <title>酒款地圖 - iWine -- 禁止酒駕．未滿18歲禁止飲酒</title>
	...
	<?php include('header_20140903.php'); ?>
                ...
                <div class="span9">
                    <div>
                        <ul class="breadcrumb">
                            ...
                            <li class="active">酒款地圖</li>
                        </ul>
                        <div class="row">
                        <div class="span9">
                             <div class="title_uploadimg">
                                <div style="float:left;"><h3><img src="images/wine_icon1.png" width="35" height="35">酒款地圖</h3></div>
                                <div style="float:right; right:0px; top:0px; padding-top:20px;"><a href="wine_section_upload.php" onclick="alert('請先登入會員，謝謝！');"><input type="button" value="酒款上傳" onclick="location.href='http://iwine.com.tw/wine_section_upload.php'"></a></div>
                            </div>
		...
		<div class="ad_list" >
			<div class="number">
				<div class="pic">
				<!--tag-->
				<a href="wine-section.php?id= <?php echo $row['id']; ?>"><img src="<?php if($row['pic1']<>""){//重要當前後台的圖處於不同檔案夾且圖名相同時可以用這招
					...
					}else{ echo "icon_prev.gif";} ?>" width="250" height="193" border="0" title="更多酒款地圖" ></a>
				</div>
				<div class="title">
				<?php echo($row["n_ws"]); ?>
				</div>
				   ...
				   <div style="padding-right:5px; height:60px; line-height:20px;">
				   <?php// echo substr_utf8(strip_tags($row['n_Variety']),35) ;?><!--價格-->
				   <?php// echo substr_utf8(strip_tags($row['n_poo']),35) ;?><!--原產地-->
				   <?php// echo substr_utf8(strip_tags($row['n_year']),35) ;?><!--年分-->
				   <?php// echo substr_utf8(strip_tags($row['n_cuisine']),35) ;?><!--適合料理-->
				   <?php if (preg_match("/日式料理/i", '"'.$row['n_cuisine'].'"')) {
				   ...
				   </div><!--這邊是葡萄品種抓取內文35個字 strip_tags()去除字串中HTML 標籤 substr_utf8參考http://www.wibibi.com/info.php?tid=122-->
				   
				   <div style="z-index: 9999; overflow:hidden; height:20px; margin-top:-20px !important;">
						<div style="float:left; z-index: 999;">
						<a href="#" onclick="window.open('https://www.facebook.com/sharer/sharer.php?u='+encodeURIComponent('http://www.iwine.com.tw/wine-section.php?id=<?php echo $row['id']; ?>'),'facebook-share-dialog','width=626,height=436');return false;"><img src="images/fb.jpg"></a>
						</div>
