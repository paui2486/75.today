...
if($_SESSION['BMEM_ID']!=""){ $BMEM_ID = $_SESSION['BMEM_ID']; }
if($_SESSION['MEM_TYPE']!=""){ $MEM_TYPE = $_SESSION['MEM_TYPE']; }
if (isset($_POST["check_form"])) {
  $title = $_POST['title'];
  $insertSQL = sprintf("INSERT INTO symposium (enrollment, area, title, start_date, pic1, contain_html, creator, creator_type, create_time, contacter, contact_email, contact_phone )
                       ...
                       GetSQLValueString($BMEM_ID, "int"),
                       GetSQLValueString($MEM_TYPE, "text"),
  ...
  $_new_id = mysql_insert_id();
    ...
    <body>Hi <br>請至後台審核品酒會 <b> ".$title." </b>。
    <br>點選聯結：<a href=\"http://admin.iwine.com.tw/qpzm105/symposium_s.php?id=$_new_id\">http://admin.iwine.com.tw/qpzm105/symposium_s.php?id=$_new_id</a>
    <br><br>煩請確認以下欄位資料便能順利符合手機app需求與網站功能運作<br>
    1.審核狀態（是否顯示於前台）<br>
    2.地點（影響手機app地址抓取，輸入地址如：台北市基隆路一段163號）<br>
    3.區域（影響搜尋功能）<br>
    4.品酒會日期時間<br>
    5.是否有封面圖片<br>
    <br>感謝。</body></html>";
...
$mail->Password   = "coevo53118909";        // SMTP account password
    ...
    $mail->AddAddress("beno@coevo.com.tw");
    $mail->AddAddress("ben@iwine.com.tw");
    $mail->AddAddress("eric@iwine.com.tw");    
    $mail->AddAddress("iwantmywine@gmail.com");
    $mail->AddAddress("beno@coevo.com.tw");
    $mail->AddAddress("eric@coevo.com.tw");
    $mail->AddBCC("service@iwine.com.tw");
    $mail->Subject    = "[iWine 品酒會審核通知]標題：".$title." ".$_today;
