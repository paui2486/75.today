...
if ($securimage->check($_POST['capacha_code']) == false) {
  msg_box('驗證碼錯誤,請重新輸入!');
  $_err_url = "regist_supplier_simple.php";
  go_to($_err_url);
  exit;
}
...
if($totalRows_member_check > 0){
	...
	go_to('regist_supplier_simple.php');
	exit;
}
...
if($_POST['check_form'] == "Y"){ 
 ...
 $insertSQL = sprintf("INSERT INTO wine_supplier (account, password_md5, company_name, email, last_login, regist_date) 
                    VALUES ('%s', '%s', '%s', '%s', '%s', '%s')",
